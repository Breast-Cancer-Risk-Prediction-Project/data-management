    ...
    for (b in eth_name){
      for ( c in study_name){
        ...
        colnames_df2 <- c("age<20", "age20.29","age30.39", "age40.49", "age50.59", "age60.69", "age70.79", 
                          ...
                          create_variables_from_levels(data,"detection_detail_primary1"),
  ...
  for (a in race_name){
    for (b in eth_name){
      for ( c in study_name){
        ...
        df <- creating_variable_summary(variable_name_string = "detection_detail_primary1",df = df,data =data, study1 = c,ethnicity1 =eth1[[b]], race1 = race1[[a]], row = row)
...
cps2_core_df <- change_case_match(data_dict = core.data.dict,
                               df = cps2_core_df)
...
cps2_incident_df <- change_case_match(data_dict = incident.data.dict,
                                  df = cps2_incident_df)
